...
//    configLocalNotification();
    ...
    firebaseMessaging!.getToken().then((pushToken) async {
      ...
      if (currentUserId != null && currentUserId!.length > 0) {
        await UsersBloc.getInstance()!
            .updatePushToken(currentUserId, pushToken);
      }
    }).catchError((err) {
...
//      Fluttertoast.showToast(msg: err.message.toString());
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      print(
          'MainScreenState　-　firebaseMessaging - onMessage: ${getScreen(message)}');

      setState(() {
        groupId = getGroupId(message);
        if (getScreen(message) == '/home' &&
            groupId != null &&
            groupId!.length > 0) {
      ...
      print(
          'MainScreenState　-　firebaseMessaging - onMessage: $newNotificationChat ');
...
//    registerNotification();
  ...
  Future<dynamic> setScreenPrefs() async {
    ...
    print(
        "MainScreenState　-　setPrefs - prefsInit         : ${prefs.getBool(Config.prefsInit)}");
  }
  ...
  Future<dynamic> setPrefs() async {
    prefs = prefs ?? await SharedPreferences.getInstance();
    await prefs.setBool(
        Config.prefsNewNotificationChat, newNotificationChat ?? false);
//    await prefs.setBool(Config.groupId, groupId ?? null);

    print(
        "MainScreenState　-　setPrefs - newNotificationChat: ${prefs.getBool(Config.prefsNewNotificationChat)}");
//    print("MainScreenState　-　setPrefs - groupId: ${prefs.getBool(Config.groupId)}");
  ...
  void _navigateToItemDetail(Map<String, dynamic> message) async {
    print("MainScreenState　-　_navigateToItemDetail - route               : " +
            getScreen(message) ??
        "/home");
//    print("MainScreenState　-　_navigateToItemDetail - url                 : " + getScreen(message) == "/latest" ?  Platform.isAndroid ? message['data']['url']: message['url'] : "blank");
    print("MainScreenState　-　_navigateToItemDetail - currentUserId       : " +
            currentUserId! ??
        "");
    print("MainScreenState　-　_navigateToItemDetail - newNotificationChat : " +
        newNotificationChat.toString());
...
//    groupId = "0TbNumLeDage4RsA0jpm";
    ...
    print(
        "MainScreenState　-　MainScreen - build - currentUserId: $currentUserId");
    print("MainScreenState　-　MainScreen - build - initScreen: $initScreen");
    print(
        "MainScreenState　-　MainScreen - build - newNotificationChat: $newNotificationChat");
    ...
    return new MaterialApp(
      ...
      routes: <String, WidgetBuilder>{
        '/home': (context) => MyAppMain(
            ...
            initScreen: initScreen ?? 0,
            newNotificationChat: newNotificationChat ?? false,
            groupId: groupId),
